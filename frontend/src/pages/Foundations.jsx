...
export default function Foundations() {
  return (
    <div className="w-full pb-24 framer-animate relative">
      ...
      <div className="max-w-[1400px] mx-auto px-6 mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {foundationsData.map((ngo, index) => (
          <AnimatedSection key={ngo.id} delay={index * 0.1} direction="up" className="bg-[#0a0a0c] border border-white/10 p-8 md:p-10 rounded-3xl hover:border-pink-500/40 transition-all group flex flex-col items-center text-center shadow-lg h-full">
            
            <Tilt glareEnable={true} glareMaxOpacity={0.4} scale={1.05} transitionSpeed={400} tiltMaxAngleX={15} tiltMaxAngleY={15}>
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border-4 border-white/10 group-hover:border-pink-500 transition-colors mb-6 shadow-2xl flex-shrink-0 relative bg-white/5 cursor-pointer">
                <img 
                  src={ngo.image} 
                  alt={ngo.founder} 
            ...
            <h3 className="text-2xl md:text-3xl font-black text-white mb-2">{ngo.name}</h3>
            <p className={`text-xs md:text-sm font-bold uppercase tracking-widest leading-relaxed mb-1 ${ngo.textClass}`}>
              {ngo.founder}
            </p>
            <span className="bg-white/10 text-slate-300 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest mb-6 inline-block border border-white/5">
              {ngo.category}
            </span>
